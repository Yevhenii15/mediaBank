        ...
        <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm bg-white border-r overflow-y-auto">
          <div class="flex items-center  border-b-border border-b py-6 px-4">
            ...
            <button class="navbar-close" @click="toggleMenu">
              <svg class="h-8 w-8 text-black cursor-pointer hover:text-gray-500" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
          ...
          <ul>
            ...
            <li class="text-p py-3 px-4 border-b-border border-b "><button class="uppercase  font-futura "
                @click="logOut">Log Out</button></li>
          </ul>
        </nav>
      </div>
    </div>
